...
export const ProviderUser = ({ children }) => {
    ...
    const SignIn = async (data) => {
        ...
        const response = await bicoApi.post("/users", data)
    ...
    const Login = async (data) => {
        ...
        const response = await bicoApi.post("/login", data)
            .then((res) => {
                setUserLogin(res.user)
                setToken(res.accessToken)
                localStorage.setItem("@user:Bico", JSON.stringify(res.user))
                localStorage.setItem("@token:Bico", res.accessToken)
